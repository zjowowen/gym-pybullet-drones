...
def run(rllib=DEFAULT_RLLIB,output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO):
    ...
    if not rllib:
        ...
        best_model_reward=-99999999999
        i=0
        while(best_model_reward<-100):
            i+=1
            results = agent.train()
            ...
            if best_model_reward+50<results["episode_reward_min"]:
                #os.mkdir("./Takeoff_rllib_ppo/checkpoint-{:d}".format(i))
                agent.save("./Takeoff_rllib_ppo/checkpoint-{:d}".format(i))
                #agent.restore("./Takeoff_rllib_ppo/checkpoint-98/checkpoint_000098/checkpoint-98")
    ...
    for i in range(3*env.SIM_FREQ):
        ...
        if done:
            obs = env.reset()
    print("[INFO] {:d}: total_reward {:f}".format(0,total_reward))
    env.close()
